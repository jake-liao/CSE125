...
class PipelinedCPU(implicit val conf: CPUConfig) extends Module {
  ...
  val booth      = Module(new Booth())  //initalizes Booth multiplier
  ...
  //Booth mulitplier value holders, necessary due to forwards being unstable
  val Booth_inx = RegInit(0.U)
  val Booth_iny = RegInit(0.U)
  ...
  when (hazard.io.idex_bubble) {
    //Needs to maintain consistent regsiters to write to for booth stalling/bubbling
    when(aluControl.io.mult === 1.U){
    ...
    //id_ex.wbcontrol := 0.U.asTypeOf(new WBControl)
    id_ex.writereg := id_ex.writereg  //needs to hold values so booth keeps same operands for 8 cycles
    id_ex.rs1 := id_ex.rs1
    id_ex.rs2 := id_ex.rs2
  ...
  branchCtrl.io.inputx := alu_inputx
  branchCtrl.io.inputy := Mux(id_ex.excontrol.immediate,id_ex.imm, alu_inputy)

  // Set the ALU operation
  alu.io.operation := aluControl.io.operation
  
  // Set up for booth multiplier
  when(ex_mem.mcontrol.taken === 1.U){
    booth.io.operation := 0.U //prevents stalls when branch is found to be taken
  } .otherwise{
    booth.io.operation := aluControl.io.mult  //Sets alu control and primary control unit for booth multiplier
  }
  booth.io.Mc        := Booth_inx   //sends in rs1 forwarded and muxed, same as ALU 
  booth.io.Mp        := Booth_iny   //sends in rs1 forwarded and muxed, same as ALU
  printf("\nMc:%b",booth.io.Mc)
  printf("\nMp:%b",booth.io.Mp)
  hazard.io.mult_done := booth.io.done    //control signal stalls pipleline, sends nops but keeps data for booth to use until done signal goes high
  hazard.io.mult     := aluControl.io.mult  //indicates that a stall needs to occur next cycle for the booth 
  printf("\nalucontrol mult: %b",aluControl.io.mult)
  printf("\nboothio ready: %b",booth.io.ready)
  when(aluControl.io.mult === 1.U && booth.io.ready === 1.U){   //stores the initial values of unstable forwarded values
    Booth_inx := alu_inputx
    Booth_iny := Mux(id_ex.excontrol.immediate, id_ex.imm, alu_inputy)
  } .otherwise{
    Booth_inx := Booth_inx
    Booth_iny := Booth_iny
  }
  ...
  when(aluControl.io.mult === 1.U){
    ex_mem.aluresult := booth.io.result.asUInt
  }.otherwise{
    ex_mem.aluresult := alu.io.result
  }
  //ex_mem.aluresult  := Mux(aluControl.io.mult,booth.io.result.asUInt,alu.io.result) //muxes either alu result of booth result
